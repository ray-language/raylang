...
- **Nativo: patrones de enum anidados y guardas `if` en `match`** (M287, ray-sublime §102).
  ...
  --native` (E0308); y una guarda `patrón if cond =>` era «not supported». Ahora ambos compilan,
  con cualquier profundidad de anidamiento, y salen byte-idénticos a la VM.
...
- **`net/webserver` 0.3.3: `send_response_for(req, conn, r)`** (M283, raystream [23]). En el camino
  ...
  HEAD; la variante nueva manda las cabeceras completas y ningún cuerpo, también con streams y
  cuerpos-fichero.
...
- **Métodos de trait del mismo nombre encadenados** (M267, IDEAS §92). `r.unwrap_or(x).unwrap_or(y)`
  ...
  posición y nombre; el checker ahora distingue el sitio por su profundidad en la cadena. Los tres
  motores dan lo mismo.
...
- **`std/ui`: evento `"focused"` y ventana en el evento `"menu"`** (M252, ray-sublime #76). Con
  ...
  lo emite en `focus` (abrir no produce eventos). Tres motores.
...
- **Devtools del webview** (M231): bajo `ray dev` y con `ray run --devtools` el inspector va
  encendido ("Inspect Element", F12; en macOS también el menú Develop de Safari). Un binario
  nativo solo lo lleva si se construyó con `ray build --native --devtools` / `ray bundle
  --devtools`: sin el flag la llamada no existe y ningún entorno puede encenderlo (Windows lo traía
  encendido de fábrica: ahora va apagado). `--ios|--android --devtools` deja el shell móvil
  inspeccionable desde el escritorio (Safari Develop / chrome://inspect).
