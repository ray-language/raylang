...
| **`@derive(ToJson)`** (serialización JSON derivada) | Checker (`generate_derives`, como Eq/Show) | **candidata** | 📌 anotada en M93.2d; **M93.4 (decisión tomada): el trait `ToJson` SUBIÓ a `std/json`** (con impls de primitivos; `web/framework` lo reexporta con `pub from` y conserva `r.json_of(v)` por bounds) → el derive ya no dependería de un paquete. Falta solo el derive en sí: generar el `impl` para structs/enums (campos → claves, strings escapados vía `stringify`) |
