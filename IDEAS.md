...
| **Nativo: cola p99 bajo alta concurrencia** | Runtime nativo (¿registro global de handles / pool mutex? psynch unfair) | **investigada (M96b), ABIERTA** | 📌 medida a fondo (18 jul): la cola es CONSTANTE ~80 ms al 1% e independiente de la concurrencia (c=50: p50 387 µs / p99 86 ms; c=500: p99 69 ms). DESCARTADOS por experimento: estampida del condvar global (fix aplicado igualmente: generación atómica + notify solo con esperadores), Nagle/delayed-ACK (NODELAY aplicado igualmente, mejora p90), sobresuscripción (c=50 ya la tiene), latidos del cv_wait (1 ms no cambia nada). Sospechosos restantes: el Mutex GLOBAL del registro de handles (2-3 locks por petición ≈ 250k/s; psynch es unfair → esperador aparcado puede ayunar ~decenas de ms) y el mutex del pool. Siguiente herramienta: trazas de latencia DENTRO del runtime (no sampling) |
