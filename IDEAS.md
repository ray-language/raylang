...
| **Cliente DNS** (resolución sobre UDP) | Librería raylang sobre los sockets UDP de M20 | **M22** | ✅ DESIGN §31. `dns.ray` (RFC 1035): resuelve registros A por UDP; `build_query`/`parse_response`/`query_a`. La pieza difícil = **compresión de nombres** (punteros `0xC0`; `read_name` los sigue y devuelve la posición siguiente). Verificado e2e contra un servidor DNS de juguete (con puntero de compresión) por ambos motores + comprobado contra DNS real (8.8.8.8). Diferido: AAAA/CNAME/MX, TCP fallback, caché por TTL |
