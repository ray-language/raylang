...
| **`@derive(ToJson)`** (serialización JSON derivada) | Checker (`generate_derives`, como Eq/Show) | **candidata** | 📌 anotada en M93.2d: el trait `ToJson` vive en `web/framework` (`r.json_of(v)` por bounds); derivar el `impl` para structs/enums (campos → claves, strings escapados vía std/json) sería el cierre redondo. Decisión pendiente: ¿el trait sube a `std/json` para que el derive no dependa de un paquete? |
