...
| **Nativo: cola p99 bajo alta concurrencia** | Runtime nativo (pool M96 / condvar global / scheduler) | **candidata (perf)** | 📌 línea base medida (18 jul 2026, M2 Max, `wrk -t4 -c500`, JSON): el webserver pelado da 93.7k req/s con p50 2.21 ms — mejor mediana que node/bun — pero **p99 73 ms** vs 5-9 ms de bun/go (framework: p99 196 ms). Sospechosos: contención del pool/`__RAY_ACT_CV` global (todo send/close/fin-de-tarea despierta a TODOS los que esperan actividad) y el spawn-por-petición. Perfilar antes de tocar |
| **Framework: App por CONEXIÓN en nativo** (`catch_unwind` en vez de task por petición) | net/webserver + preámbulo nativo | **candidata (perf)** | 📌 diferido de M93.3 con línea base (18 jul 2026): pelado 93.7k vs framework 61.5k req/s — el gap es reconstruir la App POR PETICIÓN (registro + compilar las regex de `GET_re`) porque el aislamiento panic→500 spawn-ea cada petición. Si el nativo aísla el panic con `catch_unwind` en el MISMO hilo (sin cruzar valores), el handler puede construirse una vez por conexión (`serve_with` ya existe) y reusar en el keep-alive |
