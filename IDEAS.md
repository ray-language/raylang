...
| **Cliente DNS** (resolución sobre UDP) | Librería raylang sobre los sockets UDP de M20 | **M22** | ✅ DESIGN §31. `dns.ray` (RFC 1035): resuelve **A/AAAA/MX** por UDP; `query`/`query_a`/`query_aaaa`/`query_mx` → `enum Record`. La pieza difícil = **compresión de nombres** (punteros `0xC0`; `read_name` los sigue y devuelve la posición siguiente; también en el RDATA del MX) + IPv6 canónica con `::` (RFC 5952). Verificado e2e contra un servidor DNS de juguete (A/AAAA/MX según QTYPE, con compresión) por ambos motores + comprobado contra DNS real (8.8.8.8). Diferido: CNAME/TXT, TCP fallback, caché por TTL |
