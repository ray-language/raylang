...
|------|--------------|--------|--------|
| Concurrencia (goroutines / async / suspend) | **Arquitectura de la VM** | **M12** | 🚧 dirección fijada (DESIGN §21) + **M12.1 especificado** (§21.2): **CSP sobre la VM** — green threads cooperativos M:1, canales tipados, structured concurrency; data-race freedom **vía CSP** (no ownership); scheduler determinista; intérprete = oráculo secuencial. Surface: `spawn(closure)`, `channel()->Channel<T>`/`send`/`recv->Option<T>`/`close` (builtins). Sub-fases: **M12.1** slice CSP (canales no acotados) · **M12.2** acotados/backpressure · **M12.3** structured concurrency (scope+join) · **M12.4** `select`. Diferido: algebraic effects (intérprete a pila explícita), M:N paralelo (GC thread-safe). Descartado: ownership/regiones |
