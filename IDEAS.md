...
| **Nativo: pool de hilos para `__ray_spawn`** | Preámbulo del transpilador (runtime de concurrencia) | **candidata (perf)** | 📌 medido bajo `wrk -c500` (M93.6): el backend nativo crea un HILO del SO por spawn — y el webserver spawn-ea por petición (panic→500) → ~17k thread-spawns/s; con 500 conexiones el throughput CAE vs 250 (17k vs 31k req/s) y aparece cola de latencia >2s. Un pool de workers reutilizables para las tareas cortas (las de conexión pueden seguir dedicadas) debería recuperar el gap. Medir antes/después |
