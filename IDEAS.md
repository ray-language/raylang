...
## 77b. `std/image` — decodificar PNG sobre el inflate existente (ago 2026) — ✅ HECHA (M144, DESIGN §141); codificar `encode_png` ✅ HECHA (M164, DESIGN §157 — queja del juego 1942: los sprites se generaban con Python)
...
| D decisión | ✅ M191 `break`/`continue` en forma mínima (la bandera no corta el cuerpo: bug real ×3) · ✅ M196 `pub fn` con nombre de builtin en módulo cualificado (escape `builtin.f`) · closure sin anotación con valor | la decide el usuario |
...
## 89. Auto-actualización de apps: `std/update`, manifiesto firmado y `ray release` (sep 2026) — ✅ HECHA (M246–M249, DESIGN §235–§238; publicada en v1.21.0)
...
| 2 | **Sin derivación de claves para contraseñas** en `std/crypto` (solo SHA/HMAC/HKDF): la stdlib induce `sha256(password)` | ✅ **M290**: `pbkdf2_hmac_sha256` (builtin `ring`, tres motores) + `password_hash`/`password_hash_with`/`password_verify` en raylang (formato `$pbkdf2-sha256$…`, sal CSPRNG, 600k iteraciones, `constant_time_eq`); MANUAL «Contraseñas: `password_hash`, nunca `sha256`» |
| 3 | **`ray_run` del MCP confina CPU/memoria/tiempo, no el I/O**: el snippet (escrito por un modelo) tiene disco, red y procesos del usuario | ✅ **M294** documentado en SECURITY.md («Confinamiento opcional» corregido: no acota I/O; sección «Herramientas de desarrollo»). `--deny fs,net,process` sigue PROPUESTO (misma pieza que §95 P1: capacidades en el despacho de builtins) |
| 4 | **`std/json` sin límite de profundidad**: en la VM lo corta el límite de marcos; en NATIVO no hay límite → `[[[[…` contra cualquier API nativa = stack overflow = aborto del proceso | ✅ **M291**: `json.max_depth()` = 200 (función, no `const`: el parser autoalojado no soporta constantes), `Err("nesting too deep (possible DoS): more than 200 levels")` en `parse`/`parse_relaxed`; medido antes: nativo 100 000 niveles → «has overflowed its stack» (aborto); golden en `tests/json_cli.rs` |
...
| 10 | **Servidor local de una app de escritorio/móvil expuesto a otros procesos y a páginas web** (pregunta del usuario, 25 sep): el puerto se adivina o se escanea; una página en el navegador envía POSTs y abre WebSockets (sin CORS) contra `127.0.0.1:puerto`; otra app del móvil conecta igual; el webserver no comprobaba `Origin`/`Host` ni tenía secreto | ✅ **M297** (net 0.3.5, web 0.4.4): `webserver.local_token()` + `local_limits(token)` (token por cabecera/cookie/query, cookie `SameSite=Strict`, + guarda de origen por loopback) + `web.listen_local`; `ray://app` documentado como camino por defecto; SECURITY.md «Servidores locales». Opt-in a propósito (una guarda siempre activa rompía CORS y proxies sin `X-Forwarded-For`) |
