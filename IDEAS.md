...
| C stdlib | `std/inflate` incremental (ZRLE/Tight, permessage-deflate) · ✅ M194 `std/crypto/{md5,aes,des}` (MD5/DES/3DES/AES, código y vectores ya en la app) · ✅ M195 `std/bigint` con `modpow` en runtime (`num-bigint`; 5 s → ms al conectar) | **alta** para protocolos reales |
