...
| `std/json` | `enum Json` (`JNull JBool JNum JStr JArray JObject`) · `parse -> Result<Json, string>` · `stringify` (canonical, sorted keys). `\uXXXX` escapes with surrogate pairs · M218: **`parse_relaxed(s)`** accepts `//` `/* */` comments and trailing commas (the dialect of `tsconfig.json`, `.vscode/*.json`, `.sublime-settings`); `parse` stays strict |
