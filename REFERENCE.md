...
| `std/fs` | `read_file write_file append_file remove_file list_dir exists read_file_bytes write_file_bytes` (→ `Result`) · handles: `open(path, "r"/"w"/"a") -> Result<int, _>` `read_line(h) -> Option<string>` `write(h, s)` + `close(h)` · **streaming** (M113): `read_bytes(h, max) -> Result<Option<bytes>, string>` (hasta `max` octetos desde la posición actual — exactos salvo cerca del final; `None` = EOF; memoria acotada por lo leído) · `seek(h, pos) -> Result<int, string>` (posición absoluta desde el inicio; devuelve la nueva posición → transferencias reanudables) |
