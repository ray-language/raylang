...
| `std/regex` | motor Thompson NFA (tiempo lineal): `full_match search find find_str find_all replace_all`. Soporta `. * + ? \| ( ) [a-z] [^…] \d \w \s ^ $`, `(?:…)`, lazy `*?`, `{n,m}` · **No soporta y `compile` rechaza con `Err` con nombre** (M211): look-around `(?=` `(?!` `(?<=` `(?<!`, grupos atómicos `(?>`, flags en línea `(?i)`, `\p{…}`, backreferences `\1`/`\k<n>`, `\G` `\A` `\z`, `\h`, cuantificadores posesivos `*+`, clases POSIX `[:alpha:]` y `&&` (cualquier otro escape `\X` es el carácter literal `X`; `\b` es la letra b) · compilado: `compile -> Result<Regex, string>` + trait `Matcher` (`captures`/`captures_str`) · grupos con NOMBRE `(?P<n>…)`/`(?<n>…)` (M128): `group_names(re) -> [string]` · `captures_map(re, s) -> Option<Map<string, string>>` |
