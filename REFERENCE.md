...
| `==` `!=` | igualdad estructural (primitivos, strings, bytes; tipos de usuario vía `Eq`) · M316: `f() == Option.None` / `r != Result.Err("x")` toman `T` del otro operando |
...
| Tupla | `(1, "a")` | acceso `t.0`, `t.1`; destructuring `let (a, b) = t;`; se muestra `(1, a)` en `print`/`to_string`/`assert_eq`, igual en los tres motores (M316; antes `print` daba `[[1, a]]` en la VM) |
...
| `platform` | `() -> string` | M235: el sistema operativo del proceso — `"macos"`, `"linux"`, `"windows"` (los nombres de `std::env::consts::OS`); en el nativo es un literal del target |
| `arch` | `() -> string` | M247: la arquitectura de la CPU como la nombra Rust: `"aarch64"`, `"x86_64"`. Con `platform()` forma la clave `<platform>-<arch>` del manifiesto de actualización |
...
| `s.last_index_of(sub)` | `Option<int>` | índice (de carácter) de la ÚLTIMA ocurrencia (M304; `Some(len)` con `sub` vacío) |
...
|---|---|---|
| `b.len()` | `int` | octetos |
| `b[i]` | `int` | octeto (0–255), en O(1) (a diferencia de `s[i]`) |
| `b.sub_bytes(i, j)` | `bytes` | rebanada `[i, j)` con *clamp* |
| `b.index_of(needle)` | `Option<int>` | índice de OCTETO de la primera ocurrencia de la subsecuencia (`Some(0)` con aguja vacía); sin asignar (M245: el `\r\n\r\n` de una cabecera es una llamada) |
| `b.index_of_from(needle, start)` | `Option<int>` | como `index_of` desde el octeto `start` (M304; sin copiar: el `find(buf, from)` de un parser incremental) |
| `b.last_index_of(needle)` | `Option<int>` | índice de octeto de la ÚLTIMA ocurrencia (M304; `Some(len)` con aguja vacía) |
| `b.starts_with(prefix)` | `bool` | ¿empieza por `prefix`? (el prefijo vacío siempre casa) (M245) |
...
|---|---|---|
| `Eq` | `eq(self, other: Self) -> bool` | habilita `==`/`!=` en tipos de usuario; `Option<T>`/`Result<T, E>` lo implementan si sus parámetros lo hacen (M309: `assert_eq(o, Option.Some(1))`), igual que `Show` (`Option.Some(1)`); derivable |
...
| `Option<T>` | `map(f: fn(T) -> U) -> Option<U>` / `and_then(f: fn(T) -> Option<U>) -> Option<U>` | transforma / encadena un paso falible (M304) |
| `Option<T>` | `unwrap_or_else(f: fn() -> T) -> T` | el valor o `f()` (el default solo se calcula si hace falta; M304) |
...
| `Result<T, E>` | `map(f: fn(T) -> U) -> Result<U, E>` / `and_then(f: fn(T) -> Result<U, E>) -> Result<U, E>` | transforma / encadena el `Ok`; el `Err` pasa intacto (M304) |
| `Result<T, E>` | `map_err(f: fn(E) -> F) -> Result<T, F>` / `unwrap_or_else(f: fn(E) -> T) -> T` | transforma el error / el valor o `f(e)` (M304) |
...
| `std/net` | `tcp_connect tcp_listen tcp_accept local_port` · `tcp_connect_timeout(host, port, ms)` (M122: el intento que agote el plazo falla con el error estable `"connect timeout"` en vez de los ~75 s del SO ante un host que descarta SYNs; M306: en la VM y en el nativo con fibras la espera APARCA la fibra — el dial corre en un hilo auxiliar —; en el intérprete y el nativo hilo-por-tarea bloquea solo su hilo) · `socket_read socket_write socket_read_bytes socket_write_bytes` · `shutdown_write(h) -> Result<int, string>` (M130: half-close `SHUT_WR` — el peer ve EOF, este lado sigue leyendo; el idiom netcat/HTTP-1.0; solo TCP) · `peer_addr(h) -> Result<string, string>` (M123: la dirección del peer `"ip:puerto"` de una conexión TCP/TLS; IPv6 con corchetes) · `set_read_timeout(h, ms)` (M56.4/M121: la lectura que espere más falla con el error estable `"read timeout"`; aplica a TCP, TLS **y UDP**, y desde M306 también al `tcp_accept` de un listener) · `set_nodelay(h, on)` (M203: `TCP_NODELAY` — sin Nagle, cada escritura pequeña sale ya; TCP y TLS; total) · `set_keepalive(h, on)` (M207: `SO_KEEPALIVE` — el SO sondea la conexión ociosa y la da por caída si el peer desapareció; temporización del SO; TCP y TLS; total) · `tls_connect tls_connect_h2 tls_accept tls_upgrade` (STARTTLS) · `tls_peer_cert(h) -> Result<PeerCert, string>` (M124: el certificado del peer — `PeerCert { subject, issuer, not_before_ms, not_after_ms, san }`; "expira en N días" = `(not_after_ms - time.now()) / 86400000`; conduce el handshake pendiente, acotado a 10 s) — todo `Result`; + `close(h)` |
| `std/process` | Procesos del SO, **sin shell** (argv tipado). `run(program, args) -> Result<Output, string>` · `cmd(program, args) -> Cmd` + builder encadenable `.dir .env .env_clear .stdin(bytes)` (se escribe y se CIERRA; sin él, el hijo lee `/dev/null`) `.timeout_ms .max_output .merge_output .run()`. `Err` = solo "no se pudo lanzar"; salir ≠ 0 o morir por señal es `Ok`. `Output { exit: Exit, stdout: bytes, stderr: bytes, timed_out, truncated }` con `Exit.Code(int)` \| `Exit.Signal(int)` (nunca `128+sig`). El timeout devuelve el Output PARCIAL con `timed_out` tras matar al GRUPO del hijo; `truncated` marca el tope de captura (~16 MB por defecto). **Streaming** (VM/nativo): `.stream() -> Result<Proc, string>` con `Proc { out, err: Channel<bytes>, … }` (canales ACOTADOS = contrapresión; su cierre = fin del flujo; con merge, `err` nace cerrado) · `Proc.wait() -> Exit` (cosecha; una vez) · `Proc.kill(force)` (señal al GRUPO; no-op tras wait) · **M264**: `Proc.pid() -> int` (el pid del SO; `-1` tras `wait`) y `Proc.hangup()` (`SIGHUP` al grupo — lo que manda un emulador al cerrar la ventana: un `bash` interactivo ignora SIGTERM pero con SIGHUP sale y lo reenvía a sus trabajos; Windows: cierra la pseudoconsola, y un hijo sin pty recibe `CTRL_BREAK`; no-op tras `wait`). El maestro y el esclavo del pty nacen con `FD_CLOEXEC` (M264): ningún hijo posterior hereda el terminal de otro. **Sesión persistente** (M100 v3): `.stdin_pipe()` deja el stdin del hijo ABIERTO y `Proc.write(bytes) -> Result<int, string>` / `Proc.close_stdin()` lo alimentan mientras vive — lo que necesita un cliente MCP/LSP o un driver de REPL (escribir petición → leer respuesta → repetir). `write` escribe TODO el dato y **aparca la fibra** si el pipe se llena (contrapresión); un hijo que cerró su stdin o murió da `Err` (EPIPE visible, no un silencio); `close_stdin` ES el EOF que el hijo espera. Tiene precedencia sobre `.stdin(data)`. El proceso es HIJO DE SCOPE: una hermana que falla lo mata y cosecha, y uno sin `wait()` no sobrevive a su scope. `stream()` no tiene `timeout_ms`/`max_output` (el canal es el tope; el plazo se compone con deadline + kill). macOS, Linux y Windows (M175) · **pseudo-terminal** (M237, ray-sublime): `cmd(...).pty(cols, rows).stream()` lanza el hijo bajo un PTY (Linux/macOS: `setsid` + terminal de control; `TERM`/`COLORTERM` si faltan) — `out` trae el terminal con sus secuencias VT, `err` nace cerrado, `write` manda teclas (Ctrl-C = `0x03`, lo interpreta la disciplina de línea), `Proc.resize(cols, rows)` → `SIGWINCH`; `pty(0, …)` es `Err`; Windows (M238): ConPTY — `CreatePseudoConsole` + `CreateProcessW` con `PROC_THREAD_ATTRIBUTE_PSEUDOCONSOLE`; la salida trae VT; `out` cierra al terminar el hijo (un vigía cierra la pseudoconsola) · **M246**: `Cmd.spawn_detached() -> Result<int, string>` lanza y se olvida — el hijo **sobrevive al padre** (sesión propia por `setsid` / `DETACHED_PROCESS` sin Job Object, stdio al dispositivo nulo, sin atadura a `scope`; devuelve el pid) · `self_command() -> [string]`: la línea de comandos que reproduce este programa, honesta con el motor (`[exe]` en nativo/bundle; `[ray, "run", entrada]` bajo `ray run`/`ray dev`, con `--interp` si aplica) — `cmd(c[0], resto).spawn_detached()` relanza la app |
| `std/time` | `now monotonic monotonic_millis monotonic_nanos sleep` (`monotonic` va en **ms**; `monotonic_millis` es su alias con unidad, M245) + fechas civiles UTC (M57.1): `DateTime`, `now_utc`, `from_epoch_millis`/`to_epoch_millis`, `to_iso8601[_basic]`, `date_stamp`, `to_rfc1123`, `parse_iso8601[_millis]` (RFC 3339 con offset/fracción; también las formas básicas `YYYYMMDD`/`YYYYMMDDTHHMMSSZ` que producen `date_stamp`/`to_iso8601_basic`, y `YYYY-MM-DD` = medianoche UTC, M298), `format_duration` (`net/time` queda como reexport) · constructores de duración → **ms** (la moneda de la stdlib): `millis seconds minutes hours days` — importados sin calificar habilitan la forma UFCS `30.seconds()`, `2.hours()` |
...
| `std/crypto` | cripto de **producción** (respaldada por `ring`, tiempo constante): `sha256 sha512 sha1` (`bytes -> bytes`; `sha1` es legado, solo para protocolos que lo exigen) · **hasher incremental** (M126, para archivos grandes por trozos): `sha256_init()`/`sha512_init() -> Result<int, string>` + `hash_update(h, chunk) -> Result<int, string>` + `hash_final(h) -> Result<bytes, string>` (`final` CONSUME el handle; digest idéntico al de una pasada) · `hmac_sha256(key, msg) -> bytes` · `ed25519_public_key(seed)` / `ed25519_sign(seed, msg)` → `Option<bytes>` (`None` si la semilla no mide 32 octetos) y `ed25519_verify(pubkey, msg, sig) -> bool` (total) · `chacha20poly1305_seal(key, nonce, aad, plain)` → `Option<bytes>` (`cifrado ‖ etiqueta`) y `chacha20poly1305_open(…)` (`None` si falla la autenticación) · `random_bytes(n)` (CSPRNG) · **acuerdo de claves** (respaldado por `x25519-dalek`): `x25519_public_key(secret)` / `x25519_shared_secret(secret, peer_public)` → `Option<bytes>` (`None` si alguna clave no mide 32 octetos, o si la pública del par es de orden pequeño — salida toda-ceros) · `hkdf_sha256(salt, ikm, info, len)` → `Option<bytes>` (RFC 5869; `None` fuera de `1..=8160`; `info` distinto → clave independiente) · **contraseñas** (M290): `pbkdf2_hmac_sha256(password, salt, iterations, len)` → `Option<bytes>` (RFC 8018; `None` con `iterations` fuera de `1..=4294967295` o `len` fuera de `1..=1024`) · `password_hash(p) -> string` (`$pbkdf2-sha256$<iter>$<sal hex>$<clave hex>`, sal de 16 octetos del CSPRNG, `PASSWORD_ITERATIONS` = 600000) / `password_hash_with(p, iterations)` / `password_verify(p, encoded) -> bool` (tiempo constante; `false` también ante un `encoded` malformado) · `constant_time_eq(a, b) -> bool` (total). El secreto de `x25519_shared_secret` es el DH **crudo**: pasa siempre por `hkdf_sha256` antes de usarlo como clave AEAD. Las versiones en raylang puro de `examples/web/` son demostración, no producción |
| `std/crypto/md5` `std/crypto/aes` `std/crypto/des` | cripto **LEGADA en raylang puro** (M194): para hablar con protocolos que la exigen, no para diseñar (sin tiempo constante; para lo nuevo, `std/crypto`). `md5.md5(bytes) -> bytes` (RFC 1321) · `aes.encrypt_block/decrypt_block(key, block)`, `encrypt_ecb/decrypt_ecb(key, data)`, `encrypt_cbc/decrypt_cbc(key, iv, data)` (claves de 16/24/32 octetos = AES-128/192/256; bloques enteros, sin relleno) · `des.encrypt_block/decrypt_block`, `encrypt_ecb/decrypt_ecb`, `encrypt_cbc/decrypt_cbc` (clave de 8) y **3DES** `tdes_encrypt_block/tdes_decrypt_block`, `tdes_encrypt_ecb/tdes_decrypt_ecb`, `tdes_encrypt_cbc/tdes_decrypt_cbc` (clave de 24 = K1 K2 K3, o 16 = K3 K1). Todo `-> Result<bytes, string>`; vectores oficiales en `tests/crypto_legacy_cli.rs` |
| `std/bigint` | **enteros grandes sin signo** sobre `bytes` big-endian (M195; runtime `num-bigint`, feature `bigint`, `--without bigint` en el nativo): `from_int(n) -> bytes`, `from_hex(s) -> Result<bytes,_>`, `from_bytes(b)` (normaliza), `to_hex(a) -> string`, `to_int(a) -> Option<int>`, `bit_len`, `is_zero`, `cmp(a, b) -> int` · aritmética `add sub mul div rem gcd shl shr` y **`modpow(base, exp, m)`**, `modinv(a, m)` (todas `-> Result<bytes, string>`: división por cero, resta negativa, sin inverso). Una `modpow` de 4096 bits cuesta milisegundos (antes, segundos en raylang puro). **No es de tiempo constante**: DH/RSA de cliente sí; clave privada de servidor bajo ataque de temporización no |
| `std/keychain` | **secretos en el llavero del sistema** (M266, ray-sublime §96; Keychain Services en macOS, Secret Service/libsecret por dlopen en Linux, Credential Manager en Windows; cero crates): `get(service, account) -> Result<Option<string>, string>` · `set(service, account, secret) -> Result<int, string>` (crea o reemplaza) · `delete(service, account) -> Result<bool, string>` (`true` si había algo). Los secretos son texto UTF-8 (tokens, claves de API); `service`/`account` no vacíos y sin NUL/tab/salto. `Err` si no hay llavero (sin demonio Secret Service, build slim) o el sistema deniega el acceso. **`RAY_KEYCHAIN_FILE=<ruta>`** = archivo plano 0600 como llavero (tests/CI, nunca producción). `--without keychain` en el nativo → `Err` |
...
| `std/collections/deque` | `Deque<T>`: `new len is_empty push_back push_front pop_front pop_back peek_front` · M304: `peek_back` · `get(d, i) -> Option<T>` (0 = frente) · `to_array(d) -> [T]` (copia, frente→fondo) · `iter(d) -> Iter<T>` (instantánea perezosa: `for x in deque.iter(d)`) |
...
| `std/json` | `enum Json` (`JNull JBool JNum JStr JArray JObject`) · `parse -> Result<Json, string>` · `stringify` (canónico, claves ordenadas). Escapes `\uXXXX` con pares surrogate · M218: **`parse_relaxed(s)`** acepta comentarios `//` `/* */` y comas finales (el dialecto de `tsconfig.json`, `.vscode/*.json`, `.sublime-settings`); `parse` sigue estricto · **profundidad acotada** (M291): `parse`/`parse_relaxed` devuelven `Err("nesting too deep …")` pasados `max_depth()` (200) niveles de arreglo/objeto — nunca un desbordamiento de pila (en nativo era un aborto del proceso) · M304: `Json` implementa `ToJson` → `obj().field("d", Json.JNull)` / `.field("x", parse(s)?)` incrustan un valor dinámico en el builder · M316: el builder (`obj()…render()`, `render_arr`) y `@derive(ToJson)` escriben la misma forma compacta que `stringify` (`{"k":1,"a":[1,2]}`) |
...
| `std/regex` | motor Thompson NFA (tiempo lineal): `full_match search find find_str find_all replace_all`. Soporta `. * + ? \| ( ) [a-z] [^…] \d \w \s ^ $`, `(?:…)`, lazy `*?`, `{n,m}` · **No soporta y `compile` rechaza con `Err` con nombre** (M211): look-around `(?=` `(?!` `(?<=` `(?<!`, grupos atómicos `(?>`, flags en línea `(?i)`, `\p{…}`, backreferences `\1`/`\k<n>`, `\G` `\A` `\z`, `\h`, cuantificadores posesivos `*+`, clases POSIX `[:alpha:]` y `&&` (cualquier otro escape `\X` es el carácter literal `X`; `\b` es la letra b) · compilado: `compile -> Result<Regex, string>` + trait `Matcher` (`captures`/`captures_str`) · grupos con NOMBRE `(?P<n>…)`/`(?<n>…)` (M128): `group_names(re) -> [string]` · `captures_map(re, s) -> Option<Map<string, string>>` · **dialecto ONIGURUMA** (M232, motor nativo `fancy-regex` en los tres motores; el de los `.sublime-syntax`/Ruby/TextMate): `onig(pattern) -> Result<Onig, string>` acepta look-around, backreferences `\1`/`\k<n>`, `\G`, atómicos `(?>`, posesivos, `\p{…}`, `\h` (dígito hex), flags en línea `(?i)`/`(?x)`, `&&`; `^`/`$` son anclas de LÍNEA y `.` no casa `\n` salvo `(?m)`/`(?s)` · `Onig { pattern, names: [string] }` (handle deduplicado por patrón, vive el proceso) · trait `Scanner`: `search_from(text, from) -> Option<Match>` (`\G` = `from`) · `match_at(text, at) -> Option<Match>` (anclado: el match EMPIEZA en `at`) · `is_match(text) -> bool` · `Match { start, end, groups: [Option<(int, int)>] }` (índices por carácter; `groups[0]` = todo) · `group_str(m, text, i) -> Option<string>` · `group_index(re, name) -> Option<int>` · un patrón catastrófico no cuelga: pánico `regex: backtrack limit exceeded …` al superar 10⁶ pasos · en el nativo exige la feature `regex` (con `--without regex`, `onig` cae en el stub) |
...
| `std/markdown` | Markdown → AST tipado + HTML (M111; **M241/M242: CommonMark + GFM**, verificado contra el spec de cmark-gfm — `tests/corpus/gfm_spec.json`, 595/672 ejemplos, trinquete en CI; lo que falta es HTML crudo, escapado a propósito). `parse(md) -> [Block]` · `render(blocks) -> string` · `to_html(md) -> string` · `parse_inline(s) -> [Inline]` · `normalize_label(s)`. `enum Block { Heading(int, [Inline]) Paragraph Plain (párrafo de lista APRETADA, sin <p>) Code(lang, texto) Quote([Block]) Alert(kind, [Block]) List(ordered, start, [[Block]]) Rule Table(aligns, header, rows) FootnoteDef(label, [Block]) }` · `enum Inline { Text Code Emph Strong Link([Inline], href, title) Image(alt, src, title) Strike HardBreak Check(bool) FootnoteRef(label) }` (título `""` si no hay). **Inlines (M242)**: énfasis por pila de delimitadores (`*`/`_` con flanqueo — `snake_case` va literal —, `~~tachado~~`), code spans por tiras de backticks, escapes `\`, entidades HTML5 y numéricas (`&ouml;` `&#35;` `&#x1F;`), enlaces e imágenes inline `[t](url "título")`, por referencia `[t][ref]`/`[ref][]`/`[ref]`, autolinks `<https://…>`/`<a@b.c>` y los extendidos de GFM (`www.`, `http(s)://`, correos desnudos), saltos duros (`\` o dos espacios al final de línea), URLs percent-encoded como cmark. Bloques: ATX y **setext**, código cercado (```/~~~, sangría de la cerca) y **sangrado**, citas y **avisos** `> [!NOTE]`/TIP/IMPORTANT/WARNING/CAUTION, listas con la regla real de sangría (marcador + espacios), **apretadas/sueltas**, **ítems de tarea** `[ ]`/`[x]`, continuación perezosa, reglas, tablas GFM (fila separadora bajo un párrafo de una línea), **definiciones de referencia** `[x]: url "t"` (recogidas para los enlaces), **notas al pie** `[^x]: …` (al final, `<section class="footnotes">`). HTML de salida con las convenciones del spec (`<hr />`, `<img … />`, `<li>` pegado en listas apretadas, celdas de tabla en línea propia). Fuera a propósito: bloques HTML (el HTML se escapa, política de seguridad). Subconjunto CommonMark: ATX `#`..`######`, párrafos, código cercado con lenguaje, listas anidadas (-/*/+ y `1.`; cambio de tipo = lista nueva; el número del primer marcador es el `start`: `2.` → `<ol start="2">`), citas `>`, `---`, **tablas GFM** (cabecera + separadora `|---|:--:|`; `\|` escapado; filas cortas rellenadas; sin separadora no hay tabla), diagramas **Mermaid** (una cerca ` ```mermaid ` emite `<pre class="mermaid">` con el texto escapado — el render es client-side con mermaid.js; en el AST sigue siendo `Code("mermaid", …)`), énfasis/negrita (el `_` intra-palabra NO crea énfasis — regla 17: `snake_case_name` va literal)/`código`/[enlaces]/![imágenes], escapes `\`. **Seguridad por diseño**: el HTML embebido se ESCAPA (no se interpreta) y las URLs `javascript:`/`vbscript:`/`data:` no-imagen se neutralizan a `#` — la salida se puede servir sin sanitizador. |
| `std/audio` | **salida de audio PCM** (M145): `open(sample_rate, channels) -> Result<int, string>` (dispositivo por defecto; s16le entrelazado; 8000–192000 Hz, 1–8 canales) · `write(h, samples) -> Result<int, string>` — escribe TODO y **aparca la fibra** si el dispositivo va lleno: la contrapresión ES el pacing (sintetiza tan rápido como puedas; el tempo lo pone el dispositivo) · `drain(h)` (espera a que suene lo escrito — antes de `close(h)` para un final sin corte) · **`open_latency(rate, channels, latency_ms)`** (M158: el hint dimensiona anillo/buffers/chunk y, desde M298, la cola entre el programa y el dispositivo — `write` aparca con ~la latencia encolada, sea cual sea la tasa; 20–1000 ms, 0 = default 200; en Android ≤50 ms pide LOW_LATENCY) · **`played_ms(h) -> Result<int, _>`** (M158: la posición REAL de reproducción — AudioQueueGetCurrentTime / snd_pcm_delay / AAudioStream_getFramesRead, refrescada ~latencia/4 — para sincronizar visuales) · `close(h)` (el genérico) termina la salida. Backends: AudioQueue (macOS), ALSA por dlopen (Linux; sin libasound → `Err` claro), **AAudio por dlopen (Android, M158)**, `RAY_AUDIO_SINK=null` = sumidero de tiempo real (tests/CI sin tarjeta). `--without audio` lo excluye |
| `std/ui` | **ventana + webview** (M146, IDEAS §80 F1 — la primitiva de apps de escritorio): `open(title, url, width, height) -> Result<int, string>` — ventana nativa con el webview del SISTEMA cargando `url`; **`open_with(title, url, WindowOptions)`** (M210) añade `min_width`/`min_height` (0 = sin mínimo; mayor que la ventana = `Err`), `resizable`, `center` y `autosave` (nombre bajo el que el SISTEMA recuerda tamaño y posición entre arranques — `frameAutosaveName` de NSWindow; Linux/Windows no lo tienen y lo ignoran) y **`titlebar_color`** (M224: un `#rrggbb` que pinta la barra de título del sistema con el color del tema, como Sublime — macOS: barra transparente sobre el fondo de la ventana, título claro u oscuro según la luminancia; Windows 11: color de la caption; Linux lo ignora; `""` = barra del sistema; otra forma → `Err`) y **`minimizable`** (M230: `false` deshabilita el botón de minimizar — el amarillo en macOS, `WS_MINIMIZEBOX` en Windows; GTK lo ignora — para paneles secundarios como un About); `options(width, height) -> WindowOptions` da los defaults de `open` (sin mínimo, redimensionable, centrada, sin autosave, barra del sistema, minimizable) para retocar campos (app chica: el puente IPC integrado — `window.ray.send(text)` → evento `"message"` con `tag`=texto y `window`=handle (0 en iOS), M152; no-strings viajan como JSON y `request(v)` devuelve una Promise que el programa resuelve con `as_request(e) -> Option<(int, string)>` + `reply(window, id, valor)` — M157, sobre el eval_js existente; M225: el escape del literal JS es nativo y lineal (1 MB ≈ 4 ms de ida y vuelta, antes 16 s) y **`reply_json(window, id, json)`** entrega `JSON.parse(json)`: la página recibe un objeto, no un string — y `eval_js` de vuelta; con backend: tu webserver embebido en `127.0.0.1` y el framework web como IPC) · **esquema `ray://app/…`** (M226): la página carga sus archivos directamente del proceso, sin servidor TCP local (ni puerto que otra app pueda alcanzar ni permiso de red local en el bundle) y en streaming por trozos con `Range`/206, `ETag`/304 y MIME por extensión — `mount_dir(prefix, dir) -> Result<int,_>` sirve `ray://app/<prefix>/<rel>` desde disco (ruta canónica, `..` nunca sale del directorio, un directorio sirve su `index.html`), `mount_bytes(path, bytes)` un archivo en memoria, `mount_embed(prefix, embed_prefix) -> Result<int,_>` los assets de `std/embed` (devuelve cuántos; M234: bajo `ray run`/`ray dev` monta el DIRECTORIO en vivo, en el nativo los bytes horneados) · **live-reload** (M234): bajo `ray dev` el runtime se suscribe al hub y recarga todas las ventanas al emitirse `reload` (cambio de asset embebido: sin reinicio; cambio de `.ray`: reinicio y ventana nueva); lo fija la toolchain, un binario nativo no lo mira; montar antes de que la página pida; solo GET/HEAD, lo no montado es 404; `open("App", "ray://app/index.html", …)` ya no necesita servidor; backends: macOS (WKURLSchemeHandler, streaming por trozos), Linux (M227: `webkit_web_context_register_uri_scheme` por dlopen; con WebKitGTK < 2.36 sin status/cabeceras: solo cuerpo+MIME, sin Range ni 304) y Windows (M228: registro del esquema en las opciones del entorno + `WebResourceRequested`, WebView2 ≥ 112; respuesta en memoria del tramo pedido, sin streaming) · **`focus(h) -> Result<int,_>`** (M229): trae al frente y da el foco a una ventana ya abierta (`makeKeyAndOrderFront` + activar la app en macOS, `gtk_window_present` en GTK, restaurar + `SetForegroundWindow` en Windows) — `eval_js(h, "window.focus()")` solo enfoca el documento, no la ventana nativa · `eval_js(h, js)` (fire-and-forget) · eventos (cola por proceso; kinds: `"closed"` — exactamente uno por ventana —, `"menu"`, `"message"`; headless inyecta mensajes con `RAY_UI_MSG`): `next_event() -> Result<UiEvent, string>` (la fibra aparca) · `next_event_timeout(ms) -> Result<Option<UiEvent>, string>` · `events() -> Channel<UiEvent>` (fibra-bomba; VM/nativo) · `split_events() -> (Channel<UiEvent>, Channel<UiEvent>)` (M159: `(messages, other)` — los `"message"` por el primero, el resto por el segundo; UNA fibra-bomba: consumidor único como events(), llamarlo una vez; la cola tiene **cota dura** 65536 — al llenarse cae el `"message"` más viejo, nunca un `"closed"`, con aviso stderr; solo el main frame alcanza el puente en macOS/iOS) · `close(h)` (el genérico) cierra la ventana · **menús** (M148): el menú estándar App/Edit se instala SOLO (⌘Q/⌘W y el portapapeles/undo del webview — sin él, ⌘C/⌘V no viajan en macOS); `menu(title, [MenuItem{tag,title,shortcut}]) -> Result<int,_>` añade menús custom (click → evento `"menu"` con `tag`; shortcut = un carácter ⌘+tecla en macOS, mayúscula añade ⇧; Linux v1 click-only y el menubar es por-ventana: aplica a ventanas abiertas después) · `app_menu(name, [MenuItem]) -> Result<int,_>` (M151) mete items en el **menú de APLICACIÓN** de macOS (encima de Hide/Quit, con separador) y un `name` no vacío lo re-titula (bajo `ray run` salía "ray"); el tag `"role:about"` instala el **About nativo** (panel estándar, sin evento) y `set_about(name, version, description, copyright) -> Result<int,_>` (M155) declara su contenido — nombre en negrita, línea "Version …", descripción (como créditos, al estilo Finder) y copyright; `""` omite el campo (queda lo del bundle: `ray bundle` pone nombre/versión/icono del ray.toml y el copyright de `[app] copyright`); en Linux van como menú normal titulado `name` y TODOS los items emiten el evento `"menu"` (el programa muestra su propio about) · **diálogos de archivo** (M148): `pick_file()` / `pick_folder()` / `save_file(suggested)` -> `Result<Option<string>,_>` (None = canceló; MODALES — un modal a la vez; headless los conduce `RAY_UI_PICK`). Sin `ui.run()`: el runtime captura el hilo principal solo. Backends: AppKit/WKWebView (macOS), GTK3+WebKitGTK (Linux, por dlopen — sin las libs o sin display → `Err` claro), Win32+WebView2 (Windows, M179; sin el WebView2 Runtime → `Err` claro; menús por ventana como en Linux, sin `app_menu` de sistema: sus items van como menú normal; M183: los `shortcut` son aceleradores `Ctrl+X` reales — mayúscula = `Ctrl+Shift+X` — y los diálogos son modales de la ventana), **devtools** (M231): el inspector del webview (menú contextual "Inspect Element"/F12; en macOS también el menú Develop de Safari vía `inspectable`) lo decide el **build o la toolchain, nunca el entorno**: encendido bajo `ray dev` y con `ray run --devtools`; en un binario nativo solo si se construyó con `ray build --native --devtools` / `ray bundle --devtools` (sin el flag la llamada no existe en el programa; Windows lo traía encendido de fábrica y ahora va apagado). Móvil: `ray bundle --ios|--android --devtools` deja el shell inspeccionable desde el escritorio (Safari Develop / chrome://inspect), nunca sin el flag. `RAY_UI_BACKEND=headless` = ventanas en memoria (tests/CI, cualquier OS; M217: `RAY_UI_TRACE=1` escribe en stderr cada `open`/`eval_js`/`reply` — `[ui] eval <ventana> <js>` — y `RAY_UI_EXIT_AFTER_MS=N` termina el proceso con 0 tras N ms sin eventos). `--without ui` lo excluye · **escritorio y portapapeles** (M235): `open_path(path)` (la app asociada: `open`/`xdg-open`/handler de Windows, sin shell) · `reveal(path)` (Finder `-R` / Explorer `/select,` / `FileManager1.ShowItems` con `xdg-open` del padre de respaldo); ambas `Err` si la ruta no existe · `clipboard_write(text)` / `clipboard_read() -> Result<string,_>` (NSPasteboard / GtkClipboard / CF_UNICODETEXT; sin ventana en macOS y Windows, en Linux exige sesión gráfica; headless = buffer en proceso) · **menús completos** (M236, ray-sublime #70/#68): `MenuItem { tag, title, shortcut, icon, enabled, checked }` con `item(tag, title, shortcut)` y `separator()` · `shortcut` acepta chords (`"cmd+s"`, `"cmd+alt+s"`, `"ctrl+shift+p"`, `"f5"`, `"cmd+enter"`; `cmd` = Command en macOS y Ctrl en Linux/Windows; un carácter conserva el contrato de M148; chord desconocido = `Err`; GTK sigue click-only) · `icon`: ruta de imagen o `"sf:<símbolo>"` en macOS (Windows solo `.bmp`; GTK `GtkImageMenuItem` si la lib lo trae) · `enabled`/`checked` iniciales y `set_menu_item(tag, enabled, checked)` después (Err si el tag no existe) · `menu_at(position, title, items)`: posición en la barra (macOS: tras el menú de la app, 0 = antes del Edit estándar; Linux/Windows: desde el primero; -1/fuera de rango = al final) · **`set_titlebar_color(h, color)`** (M239): la barra de título de una ventana ABIERTA en caliente — `"#rrggbb"` la tiñe como `WindowOptions.titlebar_color`, `""` devuelve la del sistema (macOS: transparencia + fondo + apariencia por luminancia; Windows 11: `DWMWA_CAPTION_COLOR`/`DWMWA_COLOR_DEFAULT`; Linux ignora); `Err` con color mal formado o ventana cerrada · **M250** `replace_menu(title, items) -> Result<int, string>`: reemplaza el contenido de un menú de nivel superior ya creado (títulos, atajos, ítems) — los tags viejos dejan de existir y los nuevos quedan vivos para `set_menu_item`; inmediato en macOS, reconstruye las barras de las ventanas abiertas en Linux/Windows; `Err` si no hay menú con ese título · **M255** (ray-sublime) **roles estándar de edición**: un item cuyo tag es `"role:undo"`, `"role:redo"`, `"role:cut"`, `"role:copy"`, `"role:paste"`, `"role:select_all"` o `"role:close"` se comporta como el del sistema (deshacer/portapapeles/seleccionar todo sobre el webview con el foco; cerrar la ventana clave) y NO emite evento; título y atajo vacíos toman el estándar (`"Paste"`, `"cmd+v"`), propios los renombran o re-asignan; `set_menu_item` por tag funciona igual; backends: macOS selector por la responder chain, Linux `webkit_web_view_execute_editing_command`, Windows `Input.dispatchKeyEvent` del DevTools Protocol (el atajo se muestra pero no se registra como acelerador: la tecla sigue llegando al webview) · `edit_menu(items) -> Result<int, string>`: el menú Edit rehecho con tus items entre los roles — en macOS reemplaza el Edit estándar en su sitio; en Linux/Windows (sin Edit por defecto) lo crea al final de la barra la primera vez y lo reemplaza después · **M256** (ray-sublime): un `shortcut` se dibuja y responde tal como se declara — macOS 12+ "localizaba" solo los de puntuación a la distribución del teclado (`cmd+/` salía `⌘'` en un teclado latinoamericano y respondía a la tecla física de la `/` US); los items lo desactivan · **M262** (ray-sublime): con el Web Inspector acoplado, redimensionar la ventana ya no deja la página cortada por arriba (macOS: el webview vive en un NSView contenedor y sigue su tamaño) · **M257** (ray-sublime) **ciclo de vida de la ventana**: `set_title(h, title)` cambia el título de una ventana abierta (`setTitle:` / `gtk_window_set_title` / `SetWindowTextW`) · `set_edited(h, bool)` el punto de "documento modificado" del botón de cerrar de macOS (`setDocumentEdited:`; Linux/Windows lo ignoran) · `intercept_close(h, bool)`: el cierre pedido por el USUARIO (botón, ⌘W/Alt+F4, WM) emite el evento **`"close_requested"`** (`window` = h) en vez de cerrar — el programa pregunta/guarda y cierra con `close(h)`, que siempre cierra (macOS `windowShouldClose:`, GTK `delete-event`, Windows `WM_CLOSE`) · `intercept_quit(bool)`: ⌘Q / Quit del menú de la app emite **`"quit_requested"`** (`window` = 0) en vez de terminar (macOS `applicationShouldTerminate:`; Linux/Windows no tienen salida de app aparte de cerrar ventanas) · todas `Err` con ventana cerrada/desconocida · **M260** **tipos de ventana y geometría**: `WindowOptions.kind` = `"document"` (la de siempre), `"panel"` (paleta de utilidad flotante sobre la app: macOS `NSPanel` utility + floating, Windows `WS_EX_TOOLWINDOW` + topmost, GTK type hint UTILITY + keep-above) o `"borderless"` (sin marco ni título; macOS con subclase que acepta el foco y movible por el fondo, Windows `WS_POPUP`, GTK `set_decorated(false)`); `always_on_top` (macOS `setLevel: floating`, Windows `HWND_TOPMOST`, GTK `keep_above`); `parent` = handle de la ventana DUEÑA (0 = ninguna; la nueva queda encima y la sigue: macOS `addChildWindow:`, Windows owner, GTK `transient_for`) · en caliente sobre una ventana abierta: `set_fullscreen(h, bool)` (macOS `toggleFullScreen:`, GTK `gtk_window_fullscreen`, Windows sin marco cubriendo su monitor con restauración), `set_always_on_top(h, bool)`, `set_size(h, w, h)` (área de contenido), `set_position(h, x, y)` (pantalla, origen arriba-izquierda; convertido para AppKit), `center(h)`, `minimize(h)`, `maximize(h)` (macOS zoom) — todas `Err` con ventana cerrada/desconocida; kind desconocido o dueña no abierta = `Err` al abrir · **M259** (ray-sublime) **menú contextual**: `popup_menu(h, items) -> Result<int, string>` muestra `items` (los mismos `MenuItem`, roles incluidos) en la posición del puntero sobre la ventana `h`; vuelve al mostrarse y la elección llega como evento `"menu"` (`tag`, `window` = h); los tags viven mientras el menú está abierto; macOS `popUpMenuPositioningItem:atLocation:inView:`, GTK `gtk_menu_popup_at_pointer`, Windows `TrackPopupMenuEx`; patrón: la página captura `contextmenu` → `window.ray.send` → `popup_menu` · **M258** (ray-sublime) **diálogos de mensaje**: `message(title, text, buttons: [string]) -> Result<int, string>` — alerta nativa MODAL con 1 a 3 botones con tus etiquetas; devuelve el ÍNDICE pulsado, y cerrar el diálogo (Esc, la X) cuenta como el ÚLTIMO botón (pon "Cancel" al final); `message_styled(title, text, style, buttons)` con `style` `"info"`/`"warning"`/`"error"` (icono/estilo del sistema); `alert(title, text)` (un OK) y `confirm(title, text, ok_label, cancel_label) -> Result<bool, string>`; backends: macOS `NSAlert`, Linux `GtkMessageDialog`, Windows `TaskDialogIndirect` (respaldo `MessageBoxW` con etiquetas del sistema por número de botones); headless: `RAY_UI_ANSWER` conduce el índice · **diálogos de archivo con opciones**: `FileDialogOptions { title, directory, suggested, filters: [FileFilter], multiple }` desde `file_options()`, `filter(name, extensions)` (extensiones sin punto) · `pick_file_with(o)` / `pick_folder_with(o)` / `save_file_with(o) -> Result<Option<string>, string>` y `pick_files(o) -> Result<[string], string>` (selección múltiple; `[]` = canceló); macOS `setAllowedFileTypes:`/`setDirectoryURL:`/`URLs`, GTK `GtkFileFilter`+`get_filenames`, Windows `SetFileTypes`/`SetFolder`/`GetResults`; headless `RAY_UI_PICK` con rutas separadas por `\n` · **M252** (ray-sublime #76): evento **`"focused"`** (`window` = la ventana que pasa a ser la clave/activa: al abrir, en `focus(h)` y cuando el usuario la trae al frente; para saber a qué ventana va una acción de menú y para refrescar al recuperar el foco) y el evento `"menu"` lleva en `window` la ventana clave en ese momento (0 si ninguna). macOS `windowDidBecomeKey:`/`keyWindow`; Linux `notify::is-active`; Windows `WM_ACTIVATE`; headless: solo en `focus` (abrir no produce eventos) · **M263 `app_url(url) -> string`** — resuelve una URL **`app://<ruta>`** a donde vive la página AHORA: bajo `ray dev` con `[frontend]` (variable `RAY_FRONTEND_URL`, que solo exporta la toolchain) a `<url del dev server>/<ruta>` (Vite con HMR); en cualquier otro caso (`ray run`, nativo, bundle) a `ray://app/<ruta>` (el build embebido montado con `mount_embed`); una URL `http://…` de tu propio webserver también cambia su origen por el del dev server bajo `ray dev` (la ruta se conserva); cualquier otra vuelve intacta. `open`/`open_with` aplican `app_url` a las URLs `app://` (una `http://` explícita se respeta tal cual) |
...
| `std/inflate` | **DEFLATE/zlib/gzip — la descompresión** (RFC 1951/1950/1952; quien busque "zlib" o "unzip": es aquí): `inflate_raw gunzip zlib_inflate` (→ `Result<bytes, string>`; formas `_limit(data, max_out)` con tope anti-bomba, default 64 MiB; M293: `max_out <= 0` es tope CERO, nunca «sin tope») · `crc32`. Sirve p. ej. para el IDAT de un PNG (`zlib_inflate`). **Incremental** (M193): `inflate_stream()` / `zlib_stream() -> InflateStream`, `stream_push(z, chunk) -> Result<bytes, string>` (devuelve lo producido por los bloques completados; la ventana LZ77 de 32 KiB sobrevive entre llamadas; cortes en cualquier octeto; error pegajoso), `stream_finished(z) -> bool`, `stream_set_limit(z, max_out)` — para un stream zlib que dura toda la sesión (ZRLE/Zlib/Tight de RFB, `permessage-deflate` de WebSocket) **M253**: `inflate_raw`/`_limit` y `crc32` usan el runtime (`miniz_oxide`) cuando el build lo trae; el código raylang sigue de respaldo (y es quien reporta los errores de un stream inválido). |
| `std/zip` | **lectura de archivos ZIP** (M219): `open(bytes) -> Result<Archive, string>` (directorio central; ZIP64/multi-disco → `Err`) · `entries(a) -> [Entry]` (`Entry { name, size, compressed_size, method, crc32, offset }`; `method` 0 = stored, 8 = deflate) · `find(a, name) -> Option<Entry>` · `read(a, name)` / `read_entry(a, e)` `-> Result<bytes, string>` (STORE y DEFLATE sobre `std/inflate`, tamaño y CRC-32 verificados; otros métodos → `Err`). Sin escritura ni cifrado. `.sublime-package`, `.jar`, `.docx`… se leen con `fs.read_file_bytes` + `zip.open` |
| `std/update` | **auto-actualización de apps** (M247, IDEAS §89): el contrato de tres capas — manifiesto `update.json` en una URL fija (`app`, `version`, `notes`, `min_version`, `artifacts` por `<platform>-<arch>` con `url`/`sha256` hex/`size`) firmado con Ed25519 en el archivo hermano `update.json.sig` (hex de la firma sobre los BYTES del manifiesto), la clave pública **horneada** en la app (`[app] public_key` del ray.toml → `ray bundle`/`build --native`; `ray run` la lee del ray.toml), y esta librería. `check(manifest_url) -> Result<Option<Release>, string>` (`None` = al día; `Err` = no verifica, malformado, o `min_version` no cumplida; `check_with(url, key)` con otra clave) · `download(release) -> Result<Package, string>` (a temporal; **verifica tamaño y sha256 antes** de devolverlo) · `apply(pkg) -> Result<string, string>` sobre `install_root()` (macOS/Linux: extrae junto al bundle y `rename` atómico, el anterior queda en `<root>.old`; Windows: los archivos en uso se renombran `*.old` y se copia encima; bajo `ray run` → `Err`; `apply_at(pkg, root)` para un directorio explícito) · `relaunch() -> Result<int, string>` (`process.self_command()` + `spawn_detached`) · `cleanup() -> int` (borra los `.old` al arrancar) · `current()` (versión horneada o `"dev"`), `app_id()`, `public_key()`, `platform_key()`, `install_root()` · utilidades: `compare_versions`, `verify_signature`, `parse_manifest`, `verify_package_bytes`, `http_get(url)` (GET con hasta 5 redirecciones sobre `std/net`), `to_hex`/`from_hex`. Una `url` de artefacto sin esquema se resuelve contra el directorio del manifiesto (lo que escribe `ray release` sin `--base-url`). El artefacto es un `.zip` con **un solo directorio raíz** (el `.app` o el directorio del bundle); rechaza rutas con `..`. Política fuera: la app decide cuándo preguntar |
| `std/deflate` | **DEFLATE/zlib/gzip — la compresión**: `deflate_raw gzip_compress zlib_compress` **M253**: `deflate_raw` (y `adler32`) usan el runtime (`miniz_oxide`, nivel 6) cuando el build lo trae; sin él (slim, wasm, `--without deflate`) el encoder en raylang responde igual, más lento. |
| `std/image` | **imágenes** (M144): `decode_png(data: bytes) -> Result<Image, string>` con `Image { width, height, pixels: bytes }` — la salida SIEMPRE es **RGBA8** (4 octetos/píxel, filas de arriba a abajo), venga lo que venga en el PNG. Soporta tipos de color 0/2/3/4/6 y profundidades 1/2/4/8/16 (16 bits → octeto alto), paleta + `tRNS` (alfa de paleta y color-key de 0/2 en 8/16), filtros None/Sub/Up/Average/Paeth, CRC verificado por chunk y tope anti-bomba en el zlib (vía `std/inflate`). Entrelazado (Adam7) → `Err` claro (diferido). Espíritu M64: input corrupto/truncado = `Err`, nunca un crash. **Codificar** (M164): `encode_png(img: Image) -> Result<bytes, string>` — escribe el `Image` RGBA8 como PNG (tipo de color 6, 8 bits, sin entrelazado, filtro None, zlib de `std/deflate`); `decode_png(encode_png(img))` devuelve los mismos píxeles; `Err` si `pixels.len() != width * height * 4` o las dimensiones no son positivas. Con `term.draw_png` o `fs.write_file_bytes` cierra el ciclo: generar sprites/capturas desde raylang sin herramientas externas |
...
| Arranque | `listen(build_app, host, port)` (bloquea; el builder es una fn TOP-LEVEL — la forma que también compila en nativo) · **`listen_local(build_app, listener, token)`** (M297: `listen_on` + `webserver.local_limits(token)` — el patrón de app de escritorio/móvil cerrado a UNA ventana: abre la ventana con `?ray_token=<token>`; cualquier otro proceso o página web recibe 403) · **`listen_on(build_app, listener)`** (M150: el split bind/serve — `net.tcp_listen(host, 0)` + `net.local_port` primero → el programa CONOCE su puerto sin carrera close/re-bind, y el backlog acepta desde el bind: el patrón de apps de escritorio) · `listen_tls` · `listen_graceful` · `listen_limits` |
...
| Servidor local (apps) | M297: `webserver.local_token()` (128 bits del CSPRNG, hex) · `local_limits(token)` (= `default_limits` + token obligatorio por `X-Ray-Token`, cookie `ray_local` o `?ray_token=`, que siembra la cookie `HttpOnly; SameSite=Strict`; y guarda de origen: por loopback, sin `X-Forwarded-*`, un `Origin` http(s) cuyo host ≠ `Host` → 403) · `serve_with_on_limits(listener, limits, make_handler)` · `cross_site_blocked(req) -> bool` · M316: `local_token_via(req, token) -> (bool, bool)` (aceptado, llegó-por-`?ray_token=`: para sembrar la cookie en un bucle de accept propio) · `read_request_carry(conn, limits, carry) -> Result<(Request, bytes), string>` (lectura con arrastre: el servidor atiende peticiones HTTP/1.1 pipelineadas en una escritura; para bucles keep-alive propios) |
...
| Comando | Qué hace |
|---|---|
| `ray new <nombre> [--frontend <plantilla-vite>]` | crea un proyecto (ray.toml + src/main.ray + .gitignore); **`--frontend react-ts`** (M263, cualquier plantilla de `npm create vite`) añade la sección **`[frontend]`** (comando dev + URL + build + `dist`), un `main.ray` que abre `app://index.html` y responde a `window.ray.request`, y `.gitignore` para `frontend/node_modules` y `frontend/dist`; NO corre npm: imprime los comandos que siguen (`npm create vite@latest frontend -- --template …`, `npm --prefix frontend install`, `ray dev`) |
| `ray run [archivo] [--] [args…]` | ejecuta (por defecto `src/main.ray`); resuelve dependencias; el primer `--` separa los argumentos del programa |
| `ray dev [archivo] [args…]` | como `run`, pero reinicia ante cambios en `.ray`/`.ray.html`/`ray.toml` (SIGTERM → drenado con `serve_graceful`). **M263 — `[frontend]` del ray.toml** (`dev = "npm --prefix frontend run dev -- --strictPort --port 5173 --clearScreen false"`, `url = "http://localhost:5173"` (default: la de Vite), `build = "npm --prefix frontend run build"`, `dist = "frontend/dist"`): `ray dev` lanza `dev` por el shell del sistema en la raíz del proyecto UNA vez por sesión (líder de su propio grupo de procesos; sobrevive a los reinicios del programa, su HMR mantiene la página), comprueba ANTES que `url` no esté ya ocupada por otro proceso (otro Vite, una sesión anterior → 73 con el `lsof`/`netstat` que lo muestra), espera ≤ 60 s a que `url` acepte conexiones en cualquiera de las direcciones del host —`localhost` es `127.0.0.1` y `::1`— (si el comando muere antes: 73), exporta **`RAY_FRONTEND_URL`** al programa (`ui.app_url`/`app://` resuelven ahí) y lo termina —árbol entero: `npm`→`sh`→`node`— al salir por cualquier vía (ventana cerrada, `q`, Ctrl-C); si el dev server muere solo, avisa y sigue. `ray build --native`/`ray bundle` corren `build` antes y embeben `dist` (como un `[native] embed` más); `ray run` sin `ray dev` sirve `dist` en vivo y avisa si no existe. Un binario nativo nunca mira `RAY_FRONTEND_URL` |
| `ray profile [archivo] [--json] [--out FILE] [--top N] [args…]` | M240: como `run` (solo VM) con el **perfilador por función**: al terminar (también tras `exit()` o un error en tiempo de ejecución) imprime en stderr —o escribe en `--out`— una tabla por función con tiempo propio (sin hijos), % del total, tiempo inclusivo, llamadas y media por llamada, ordenada por tiempo propio; `--json` da `{wall_ns, fibers, functions:[{name, calls, self_ns, inclusive_ns}]}`. Instrumentado en el camino de llamada (dos lecturas de reloj por llamada; apagado no cuesta nada); los builtins caen en el propio de quien los llama; en recursión el inclusivo cuenta una vez por activación externa; una llamada en COLA cierra el marco del llamador (su tiempo no la incluye) |
...
| `ray bundle [archivo] [--name N] [--icon i.png] [--id com.x.y] [-o dir] [--without lista] [--ios [--ios-target device\|sim\|both]]` | empaqueta una **app de escritorio** (M147c; M208: nombre/icono/id salen de **`[app] name/icon/id`** del ray.toml —el icono relativo a la raíz del proyecto— y los flags los sobrescriben; un flag desconocido es error 64 con el uso y `--help` imprime el uso sin compilar; M209: **`[app.plist]`** del ray.toml va tal cual al Info.plist —`clave = "texto"` → `<string>`, `true`/`false` → `<true/>`/`<false/>`— y **`NSLocalNetworkUsageDescription`** se añade sola si el programa importa `std/net`, `std/udp` o el paquete `net`, salvo que la declares tú): build nativo `--release` (con el `[native] embed` del ray.toml) → `.app` en macOS (Info.plist + icns vía sips/iconutil + codesign ad-hoc), directorio con `.desktop` en Linux o, en Windows (M180), directorio con `<name>.exe` (subsistema WINDOWS: sin consola al doble clic; icono y VERSIONINFO — nombre, versión, `[app] copyright` — embebidos como recursos por `UpdateResourceW`, sin crates) + `<name>.lnk` (destino y cwd absolutos; cópialo al menú Inicio); sin Authenticode en v1 (SmartScreen avisa); **`--ios`** (§80b) genera en su lugar el PROYECTO XCODE de una app iOS — shell WKWebView en ObjC + staticlibs de dispositivo y simulador (`ray build --native --lib` por dentro; xcconfig elige el `.a` por SDK; `--ios-target device|sim` construye solo un lado — iterando contra un destino, el otro build sobra — y el `.a` del lado no construido se PRESERVA del proyecto anterior) + Info.plist; el MISMO fuente de escritorio corre en el iPhone (`ui.open` entrega la URL al webview del shell; ciclo de vida como eventos `lifecycle`). El `.app` de macOS escribe `NSHumanReadableCopyright` desde **`[app] copyright = "…"`** del ray.toml (M155 — el panel About lo muestra). Simulador sin firma; dispositivo: declara el team en `[ios] development_team = "…"` del ray.toml (M151; el bundle lo escribe en el App.xcconfig y además PRESERVA una firma ya presente al regenerar — antes cada bundle la borraba), o ábrelo en Xcode y elígelo una vez. `--ios` excluye `process` y `audio`. **`--android`** (M156) genera el PROYECTO GRADLE (shell Java + WebView; el programa como cdylib `libray_app.so` en `jniLibs/` — los símbolos JNI van dentro), `--android-abi arm64|x86_64|all` (arm64 default; preserva el `.so` del ABI no construido y `local.properties`), `[android] application_id` en ray.toml; mismas exclusiones que iOS; stdout→logcat tag `ray`; compilar con `gradle assembleDebug` (Gradle 9.x + JDK 17+, AGP 9 pinneado); `--icon` genera los `mipmap-*/ic_launcher.png` multi-densidad (M160, vía sips; legacy — Android 8+ lo enmascara a círculo) y la **firma de release** va por `keystore.properties` en la raíz del proyecto generado (condicional, cero secretos en ray.toml; keystore + properties PRESERVADOS al regenerar — flujo completo en el README generado). OJO: el .app lanza con cwd=/ → los assets van embebidos; sin firma/notarización en v1 (macOS 15+: una app descargada sin firmar exige aprobación en Ajustes) · **M249 firma y notarización**: `--sign IDENTIDAD` (o `[app] sign`, o `RAY_SIGN_IDENTITY`) firma el `.app` con `codesign --options runtime --timestamp` y los entitlements de `--entitlements`/`[app] entitlements` (por defecto un plist vacío), verifica con `--verify --strict` y falla con 74 si no verifica; `--notary PERFIL` (o `[app] notary`, o `RAY_NOTARY_PROFILE`; el perfil se crea con `xcrun notarytool store-credentials`) somete el `.app` a Apple (`notarytool submit --wait`), exige `Accepted` y grapa el ticket (`stapler`). En Windows `--sign` es el sujeto del certificado o un `.pfx` (`RAY_SIGN_PFX_PASSWORD`) para `signtool sign /fd SHA256 /tr …`. Sin identidad, ad-hoc como antes (macOS 15+ pide aprobación). `ray release` acepta los mismos flags y los pasa al bundle |
| `ray keygen [--app-id X] [--force]` | M248: crea la clave Ed25519 de **firma de la app** (semilla hex en `~/.ray/keys/<app-id>.key`, o `RAY_KEYS_DIR`; modo 0600) y escribe la pública en `[app] public_key` del ray.toml (la crea si falta). Distinta de `ray registry keygen` (publicación de paquetes). Se niega a pisar una clave existente sin `--force`: las apps en circulación dejarían de confiar en tus manifiestos |
| `ray release [archivo] [-o dist] [--notes URL] [--min-version V] [--base-url URL] [--key HEX] [--without lista] [--publish] [--tag vX.Y.Z]` | M248: el lado del publicador de `std/update`: corre `ray bundle`, comprime el bundle en `dist/<name>-<version>-<platform>-<arch>.zip` (deflate, un solo directorio raíz, modos unix), escribe `dist/update.json` (`app`, `version`, `notes`, `min_version`, `artifacts`; **conserva los artefactos de otras plataformas de la misma versión** ya presentes: se corre una vez por plataforma sobre el mismo `dist/`) y lo firma en `update.json.sig` con la clave de `--key`, `RAY_SIGNING_KEY` o `~/.ray/keys/<app-id>.key`; se niega si la clave no es la de `[app] public_key`. Sin `--base-url` las URLs son relativas al manifiesto (sirve `dist/` tal cual); con `--publish`, `gh release create` + `upload --clobber` en el tag (`v<version>`) y las URLs apuntan a `releases/download/<tag>/` del remoto `origin` — la app consulta `https://github.com/<o>/<r>/releases/latest/download/update.json` |
| `ray test [archivo] [filtro]` | corre las `@test` del proyecto: la entrada y todos sus módulos (calificadas: `math.t`) + cada `tests/*.ray` como suite de integración; filtro por subcadena; sale con 0/1 (65 si algo no compila — **toda** suite se chequea aunque no tenga pruebas, también la entrada del proyecto, M200). **`--native [--release]`** (M312): compila cada suite a un binario nativo (un `main` que despacha por nombre de prueba; honra `[native] without`, los assets embebidos y `[app]` del proyecto) y corre cada prueba como proceso — mismo informe y códigos, sin la línea `at módulo:línea:col` (el nativo no lleva traza); combinable con `--watch` y el filtro |
| `ray fmt <archivo>... [--write]` | imprime la versión canónica (conserva `if let` — M201 — y los paréntesis y comentarios del usuario; indentación de 4; lo que pase de 100 columnas se reparte: un `from … import` a un nombre por línea, una cadena de métodos a un eslabón por línea, las cadenas de `&&`/`\|\|`/`+` a un operando por línea, y las listas delimitadas —argumentos, parámetros de `fn`, literales— a un elemento por línea con el cierre en línea propia; los comentarios trailing se quedan con su operando/elemento y tus paréntesis se conservan). `--write`/`-w` reescribe en el sitio y admite varios archivos. Formas canónicas que sorprenden (M217): un bloque vacío es `{ }` (con espacio) y entre dos `const` de nivel superior va una línea en blanco |
...
| `ray check [archivo]` | alias de `ray build` (M217): chequea sin ejecutar; 0 ok / 65 error |
| `ray doc <archivo \| std/<módulo> \| <módulo>.<símbolo> \| <builtin>>` | documentación Markdown de la superficie pública (`///`) de un archivo; con un símbolo (M217) la misma respuesta que `ray_doc` del MCP: `ray doc std/ui` lista el módulo, `ray doc ui.MenuItem` da la firma y la doc · M305: también las constantes públicas (`ray doc crypto.PASSWORD_ITERATIONS`), los módulos de colecciones por su ruta (`ray doc std/collections/deque`) y, en `ray_doc` con `path`, la superficie de un módulo del proyecto o de un paquete (`"rpc/rpc"`, `"web/framework"`) · M316: `ray doc string.last_index_of` / `bytes.index_of_from` / `Result.map` / `Option.and_then` (forma `Tipo.metodo`: función libre por su primer parámetro o método de trait del prelude; el nombre a secas lista todas las variantes); dentro de un proyecto (un `ray.toml` por encima) resuelve también sus módulos y sus dependencias de `.ray-deps`; `ray doc --help` |
| `ray serve [dir] [--host H] [--port N]` | sirve un directorio de archivos estáticos por HTTP para previsualizar (`_site/`, `playground/`, la salida de `ray doc`): `index.html` por directorio, MIME por extensión, `Cache-Control: no-store`, sin `..`; por defecto `.` en `127.0.0.1:8000`, `--port 0` elige uno libre. Es el servidor escrito en raylang embebido en el binario (M199), no un servidor de producción (para eso, `net/webserver`) |
...
| `ray version` | versión — `1.27.12` en una release; `1.27.12+dev.<sha>` compilado de la HEAD del repo (M309: la HEAD y la release aceptan código distinto; `[package] raylang = "1.27.12"` en `ray.toml` exige un toolchain ≥ esa versión y avisa cuando el toolchain es de desarrollo) |
