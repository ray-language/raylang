...
| `std/io` | escritura **sin salto de línea**: `write(s)` / `ewrite(s)` (stderr) / `write_bytes(b)` → `Result<int, string>` (nº de caracteres/bytes) · `flush() -> Result<int, string>`. stdout va con buffer: tras un `write` sin `\n`, llama `flush()` para verlo (prompts, barras de progreso, secuencias de escape); `ewrite` es visible al instante. `write_bytes` no pasa por UTF-8 (bytes crudos intactos). El orden respecto a `print`/`eprint` es el de programa |
