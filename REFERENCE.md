...
| `std/audio` | **salida de audio PCM** (M145): `open(sample_rate, channels) -> Result<int, string>` (dispositivo por defecto; s16le entrelazado; 8000–192000 Hz, 1–8 canales) · `write(h, samples) -> Result<int, string>` — escribe TODO y **aparca la fibra** si el dispositivo va lleno: la contrapresión ES el pacing (sintetiza tan rápido como puedas; el tempo lo pone el dispositivo) · `drain(h)` (espera a que suene lo escrito — antes de `close(h)` para un final sin corte) · `close(h)` (el genérico) termina la salida. Backends: AudioQueue (macOS), ALSA por dlopen (Linux; sin libasound → `Err` claro), `RAY_AUDIO_SINK=null` = sumidero de tiempo real (tests/CI sin tarjeta). `--without audio` lo excluye |
| `std/ui` | **ventana + webview** (M146, IDEAS §80 F1 — la primitiva de apps de escritorio): `open(title, url, width, height) -> Result<int, string>` — ventana nativa con el webview del SISTEMA cargando `url` (el patrón: tu webserver embebido en `127.0.0.1`; el IPC JS↔raylang es el framework web) · `eval_js(h, js)` (fire-and-forget) · eventos (cola por proceso; v1: `"closed"`, exactamente uno por ventana): `next_event() -> Result<UiEvent, string>` (la fibra aparca) · `next_event_timeout(ms) -> Result<Option<UiEvent>, string>` · `events() -> Channel<UiEvent>` (fibra-bomba; VM/nativo) · `close(h)` (el genérico) cierra la ventana · **menús** (M148): el menú estándar App/Edit se instala SOLO (⌘Q/⌘W y el portapapeles/undo del webview — sin él, ⌘C/⌘V no viajan en macOS); `menu(title, [MenuItem{tag,title,shortcut}]) -> Result<int,_>` añade menús custom (click → evento `"menu"` con `tag`; shortcut = un carácter ⌘+tecla en macOS, mayúscula añade ⇧; Linux v1 click-only y el menubar es por-ventana: aplica a ventanas abiertas después) · `app_menu(name, [MenuItem]) -> Result<int,_>` (M151) mete items en el **menú de APLICACIÓN** de macOS (encima de Hide/Quit, con separador) y un `name` no vacío lo re-titula (bajo `ray run` salía "ray"); el tag `"role:about"` instala el **About nativo** (panel estándar, sin evento); en Linux van como menú normal titulado `name` y TODOS los items emiten el evento `"menu"` (el programa muestra su propio about) · **diálogos de archivo** (M148): `pick_file()` / `pick_folder()` / `save_file(suggested)` -> `Result<Option<string>,_>` (None = canceló; MODALES — un modal a la vez; headless los conduce `RAY_UI_PICK`). Sin `ui.run()`: el runtime captura el hilo principal solo. Backends: AppKit/WKWebView (macOS), GTK3+WebKitGTK (Linux, por dlopen — sin las libs o sin display → `Err` claro), `RAY_UI_BACKEND=headless` = ventanas en memoria (tests/CI, cualquier OS). `--without ui` lo excluye |
...
| `ray bundle [archivo] [--name N] [--icon i.png] [--id com.x.y] [-o dir] [--without lista]` | empaqueta una **app de escritorio** (M147c): build nativo `--release` (con el `[native] embed` del ray.toml) → `.app` en macOS (Info.plist + icns vía sips/iconutil + codesign ad-hoc) o directorio con `.desktop` en Linux; **`--ios`** (§80b) genera en su lugar el PROYECTO XCODE de una app iOS — shell WKWebView en ObjC + staticlibs de dispositivo y simulador (`ray build --native --lib` por dentro; xcconfig elige el `.a` por SDK) + Info.plist; el MISMO fuente de escritorio corre en el iPhone (`ui.open` entrega la URL al webview del shell; ciclo de vida como eventos `lifecycle`). Simulador sin firma; dispositivo: declara el team en `[ios] development_team = "…"` del ray.toml (M151; el bundle lo escribe en el App.xcconfig y además PRESERVA una firma ya presente al regenerar — antes cada bundle la borraba), o ábrelo en Xcode y elígelo una vez. `--ios` excluye `process` y `audio`. OJO: el .app lanza con cwd=/ → los assets van embebidos; sin firma/notarización en v1 (macOS 15+: una app descargada sin firmar exige aprobación en Ajustes) |
