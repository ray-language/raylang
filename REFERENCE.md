...
| `std/json` | `enum Json` (`JNull JBool JNum JStr JArray JObject`) · `parse -> Result<Json, string>` · `stringify` (canónico, claves ordenadas). Escapes `\uXXXX` con pares surrogate · M218: **`parse_relaxed(s)`** acepta comentarios `//` `/* */` y comas finales (el dialecto de `tsconfig.json`, `.vscode/*.json`, `.sublime-settings`); `parse` sigue estricto |
