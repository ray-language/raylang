...
| Módulo | Qué cubre |
|---|---|
| `std/fs` | disco: leer/escribir (texto y `bytes`), `append`, `exists`, `list_dir`, metadatos, copiar/renombrar/borrar, directorios, handles con `read_line`/`write`/`read_bytes`/`seek` |
