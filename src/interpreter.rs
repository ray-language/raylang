...
impl<'a> Interpreter<'a> {
    ...
    fn eval_builtin(&self, name: &str, values: Vec<Value>) -> Value {
        match name {
            ...
            | "__remove_all" | "__temp_dir" | "__make_temp_dir" | "__real_path" | "__symlink" => {
                ...
                let op = match name {
                    ...
                    "__mtime" => FsOp::Mtime,
                    "__stat" => FsOp::Stat,
                    "__real_path" => FsOp::RealPath,
                    "__rename" => FsOp::Rename,
                    "__copy_file" => FsOp::CopyFile,
                    "__symlink" => FsOp::Symlink,
                    _ => unreachable!(),
                };
                ...
                let arr = match (&values[0], &values[1]) {
                    (Value::Str(title), Value::Array(items)) => {
                        let items: Vec<String> = items
                            ...
                            .collect();
                        match crate::builtins::ui_replace_menu(title, &items) {
                            Ok(()) => vec![Value::Str("ok".to_string())],
                            Err(e) => vec![Value::Str("err".to_string()), Value::Str(e)],
                        }
                    }
