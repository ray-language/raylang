...
mod tests {
    ...
    fn import_con_path_de_directories_y_leaf() {
        ...
        assert_eq!(i.module, "geo/formas/circle");
        assert_eq!(i.alias, None);
        assert_eq!(i.leaf(), "circle");
    }

    #[test]
    fn import_con_path_y_alias() {
        let prog = parse_prog("import geo/formas/circle as c;\nfn main() -> int { 0 }\n");
        let i = &prog.imports[0];
        assert_eq!(i.module, "geo/formas/circle");
    ...
    #[test]
    fn from_import_se_parses_con_alias_y_various_names() {
        let prog = parse_prog("from mates import double, triple as tri;\nfn main() -> int { 0 }\n");
        assert_eq!(prog.from_imports.len(), 1);
        let fi = &prog.from_imports[0];
        assert_eq!(fi.module, "mates");
        assert_eq!(fi.names.len(), 2);
        assert_eq!(fi.names[0].name, "double");
        assert_eq!(fi.names[0].alias, None);
        assert_eq!(fi.names[0].local(), "double");
    ...
    fn qualified_struct_literal_keeps_the_dot() {
        ...
        match &e.kind {
            ...
            other => panic!("se esperaba un literal de struct qualified, fue {:?}", other),
        }
    }
    ...
    fn qualified_variant_pattern_keeps_the_dot() {
        ...
        match &arms[0].pattern.kind {
            ...
            other => panic!("se esperaba un patrón de variant qualified, fue {:?}", other),
        }
    }
    ...
    #[test]
    fn function_generic_se_parses() {
        let prog = parse_prog("fn map_over<T, U>(xs: [T], f: fn(T) -> U) -> [U] { xs } fn main() {}");
    ...
    fn match_se_parses() {
        ...
        assert_eq!(
            sx(&parse_expr("match (e) { E.A(_, x) => x, other => 9 }")),
            "(match e [E.A(_, x) => x, other => 9])"
        );
    }

    #[test]
    fn function_anonymous_se_parses() {
    ...
    #[test]
    fn ty_function_se_parses() {
        let prog = parse_prog("fn applies(f: fn(int) -> int, x: int) -> int { f(x) } fn main() {}");
        let f = &prog.functions[0];
...
"#;
    ...
    #[test]
    fn pipeline_has_precedence_minima() {
    ...
    fn pipeline_compone_con_ufcs() {
        ...
        assert_eq!(
            sx(&parse_expr("x.double() |> inc")),
            "(call inc [(call (field x double) [])])"
        );
    }
    ...
    fn parse_default_method() {
        ...
        let t = &prog.traits[0];
        assert_eq!(t.methods.len(), 2);
        assert!(t.methods[0].default_body.is_none(), "req es requerido (sin body)");
        assert!(t.methods[1].default_body.is_some(), "opt has body por default");
    }
