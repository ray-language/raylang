...
struct Transpiler {
    ...
    /// M266: el programa usa `std/keychain` (`__keychain`) → feature `keychain` de ray-runtime.
...
pub fn transpile_entry(prog: &Program, exclude: &[String], fast: bool, fibers: bool, embed: &[(String, String)], lib_mode: bool) -> Result<Transpiled, String> {
    ...
    // en la VM y el nativo fallaba con E0277. Si el tipo ya DERIVA `PartialEq` (M270, igualdad
    // estructural, la misma que `values_equal` en la VM), no se emite el `eq` manual: chocaban
    // (E0119, CI 1181 tras 1.27.1) y además el derivado es el que coincide con la VM.
    t.emit_ord_impls(&mut out, prog, &eq_types);
    ...
    out.push_str(concat!(
        ...
        "                        // Pipe cerrado (`programa | head`): convención Unix — exit 141 en\n",
        "                        // silencio, no un pánico (el mismo trato que host_print en la VM).\n",
        "                        let r = out.write_all(&buf).and_then(|()| out.flush());\n",
        "                        if let Err(e) = r { if e.kind() == std::io::ErrorKind::BrokenPipe { std::process::exit(141); } }\n",
        ...
        "fn __ray_eprint(line: String) {\n",
        "    use std::io::Write;\n",
        "    let mut e = std::io::stderr().lock();\n",
        "    let r = e.write_all(line.as_bytes()).and_then(|()| e.write_all(b\"\\n\"));\n",
        "    if let Err(err) = r { if err.kind() == std::io::ErrorKind::BrokenPipe { std::process::exit(141); } }\n",
        "}\n",
    ...
    // M266: llavero del sistema (detectado por USO de `__keychain`; `--without keychain` → Err-valor).
    if t.needs_rt_keychain {
        rt_features.push("keychain");
    }
    // M253: DEFLATE acelerado (detectado por USO de `__deflate_op`; `--without deflate` → `[]` y los
