...
",
    ...
    if t.needs_rt_ui {
        out.push_str(concat!(
            ...
            // M239: color de la barra de título en caliente.
            "fn __ray_ui_set_titlebar(h: i64, color: &str) -> Rc<std::cell::RefCell<Vec<Rc<str>>>> {\n",
            "    let known = matches!(__ray_reg().lock().unwrap().open.get(&h), Some(__RayHandle::Window(_)));\n",
            ...
            "fn __ray_ui_dialog_with(kind: &str, title: &str, dir: &str, suggested: &str, filters: &Rc<std::cell::RefCell<Vec<Rc<str>>>>, multiple: bool) -> Rc<std::cell::RefCell<Vec<Rc<str>>>> {\n",
            "    let opts = ray_runtime::ui::FileDialogOptions { title: title.to_string(), directory: dir.to_string(), suggested: suggested.to_string(), multiple,\n",
            "        filters: filters.borrow().iter().map(|f| { let (n, e) = f.split_once('\\t').unwrap_or((&f[..], \"\")); (n.to_string(), e.split(',').filter(|x| !x.is_empty()).map(|x| x.trim().to_string()).collect()) }).collect() };\n",
            "    Rc::new(std::cell::RefCell::new(match ray_runtime::ui::dialog_with(kind, &opts) {\n",
            "        Ok(paths) if paths.is_empty() => vec![Rc::<str>::from(\"none\")],\n",
            ...
            "        Ok(i) => vec![Rc::<str>::from(\"ok\"), Rc::<str>::from(i.to_string().as_str())],\n",
            "        Err(e) => vec![Rc::<str>::from(\"err\"), Rc::<str>::from(e.as_str())],\n",
            "    }))\n}\n",
    ...
    if t.needs_rt_process {
        ...
        out.push_str(concat!(
            ...
            // M264: pid y hang-up (SIGHUP al grupo / cierre de la pseudoconsola), como en builtins.rs.
            "fn __ray_proc_pid(h: i64) -> i64 {\n",
            "    let reg = __ray_reg().lock().unwrap();\n",
            "    match reg.open.get(&h) { Some(__RayHandle::Child(child)) => child.id() as i64, _ => -1 } }\n",
            "fn __ray_proc_hangup(h: i64, h_pty: i64) {\n",
            "    let reg = __ray_reg().lock().unwrap();\n",
            "    let pid = match reg.open.get(&h) { Some(__RayHandle::Child(child)) => Some(child.id()), _ => None };\n",
            "    let pty = match reg.open.get(&h_pty) { Some(__RayHandle::Pty(p)) => Some(p), _ => None };\n",
