...
fn launch() -> (Child, u16) {
    ...
    for lib in ["webserver.ray", "metrics.ray"] {
        std::fs::copy(format!("{root}/examples/web/{lib}"), dir.join(lib)).unwrap_or_else(|_| panic!("copia {lib}"));
    }
    let demo = std::fs::read_to_string(format!("{root}/examples/web/metrics_server_demo.ray")).expect("lee demo");
...
fn endpoint_metrics_expone_counters_e_histograma() {
    ...
    assert!(m.contains("Content-Type: text/plain"), "type de contenido Prometheus: {m}");
