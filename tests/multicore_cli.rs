...
fn main() -> int {
    let t0 = spawn(fn() -> int { square(0) });
    let t1 = spawn(fn() -> int { square(1) });
    let t2 = spawn(fn() -> int { square(2) });
    let t3 = spawn(fn() -> int { square(3) });
    let t4 = spawn(fn() -> int { square(4) });
    let t5 = spawn(fn() -> int { square(5) });
    let t6 = spawn(fn() -> int { square(6) });
    let t7 = spawn(fn() -> int { square(7) });
...
"#;
    let (out, err, code) = run_threads("mc_sum_tasks", src, 4);
    assert_eq!(out, "140\n", "stderr: {err}");
    assert_eq!(code, 140);
}
...
fn trabajo(iters: int) -> int {
    var sum = 0;
    var i = 0;
    while (i < iters) { sum = sum + (i * i) % 7; i = i + 1; }
    sum
}
fn main() -> int {
...
fn square(x: int) -> int { x * x }
fn main() -> int {
    let a = spawn(fn() -> int { square(6) });
    let b = spawn(fn() -> int { square(7) });
