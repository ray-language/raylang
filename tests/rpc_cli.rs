    ...
    let r = rpc.serve_shutdown("127.0.0.1", 0, stop, 2000, fn(req: rpc.Req) -> Result<Json, string> {
        if (req.method == "sum") {
            match (req.params) {
                ...
                _ => Result.Err("sum espera un arreglo"),
            }
            ...
            Result.Err("método desconocido: " + req.method)
        }
    });
    print("rpc servidor apagado");
    0
}
"#;
...
fn muestra(nombre: string, r: Result<Json, string>) {
    match (r) {
        Result.Ok(j) => print(nombre + "=" + stringify(j)),
...
fn main() -> int {
    let port = match (parse_int(args()[0])) {
        Option.Some(p) => p,
        Option.None => panic("puerto inválido"),
    };
    let c = conectar(port);
    // Lo básico: varias peticiones secuenciales por la MISMA conexión, con id correlado.
    muestra("sum", rpc.call(c, "sum", Json.JArray([Json.JNum(1.0), Json.JNum(2.0), Json.JNum(3.0)])));
    var o: Map<string, Json> = Map.new();
    o.insert("k", Json.JStr("v"));
    muestra("echo", rpc.call(c, "echo", Json.JObject(o)));
    // El traceparent y el deadline viajan en el sobre (el handler los ve en Req).
    muestra("trace", rpc.call_full(c, "trace", Json.JNull, 0, "00-abc-def-01"));
    muestra("deadline", rpc.call_full(c, "deadline", Json.JNull, 500, ""));
    // El Err del handler llega como Err del call.
    muestra("err", rpc.call(c, "nada", Json.JNull));
    ...
    muestra("sum2", rpc.call(c, "sum", Json.JArray([Json.JNum(2.0), Json.JNum(2.0)])));
    ...
    muestra("sum3", rpc.call(c2, "sum", Json.JArray([Json.JNum(5.0)])));
    // El apagado ordenado, POR RPC: el handler manda al canal stop; la respuesta llega igual.
    muestra("apagar", rpc.call(c2, "apagar", Json.JNull));
    rpc.disconnect(c2);
    0
}
"#;
...
fn proyecto(nombre: &str) -> std::path::PathBuf {
    let base = std::env::temp_dir().join(format!("ray_rpc_{nombre}"));
...
fn lanzar_servidor(base: &std::path::Path) -> (Child, u16) {
...
fn correr_cliente(base: &std::path::Path, port: u16) -> (String, i32) {
...
fn rpc_de_punta_a_punta() {
    let base = proyecto("e2e");
    let (server, port) = lanzar_servidor(&base);
    let (out, code) = correr_cliente(&base, port);
    assert_eq!(code, 0, "cliente sale 0\n{out}");
    let esperado = "sum=6\n\
echo={\"k\":\"v\"}\n\
trace=\"00-abc-def-01\"\n\
deadline=500\n\
err err=método desconocido: nada\n\
boom handler_err=true\n\
sum2=4\n\
timeout=true\n\
sum3=5\n\
apagar=\"bye\"\n";
    assert_eq!(out, esperado, "la batería del cliente, byte a byte");
    let sout = server.wait_with_output().expect("espera al servidor");
    let s_stdout = String::from_utf8_lossy(&sout.stdout);
    assert_eq!(sout.status.code(), Some(0), "el servidor sale 0 tras el apagado\n{s_stdout}");
    assert!(s_stdout.contains("rpc apagando"), "anuncia el apagado: {s_stdout}");
    assert!(s_stdout.contains("rpc servidor apagado"), "serve_shutdown devolvió a main: {s_stdout}");
}
...
fn main() -> int {
    let port = match (parse_int(args()[0])) { Option.Some(p) => p, Option.None => panic("puerto"), };
