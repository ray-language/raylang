...
fn run_selfhost(file: &str) -> (String, i32) {
    let out = Command::new(env!("CARGO_BIN_EXE_raylang"))
...
fn compare_file(rel: &str) {
    ...
    let (so_s, code_s) = run_selfhost(abs);
    assert_eq!(so_s, so_r, "stdout difiere en {rel}");
    assert_eq!(code_s, code_r, "código de output difiere en {rel}");
}
...
fn compare_source(src: &str, name_tmp: &str) {
    ...
    assert_eq!(code_s, code_r, "código de output difiere para:\n{src}");
}
...
fn arithmetic_y_control() {
    compare_source("fn main() -> int { print(2 + 3 * 4 - 1); 0 }", "in_arit.ray");
    compare_source("fn main() -> int { print(17 / 5); print(17 % 5); 0 }", "in_divmod.ray");
    compare_source("fn main() -> int { let x = 3.5; print(x + 2.0); print(x * 2.0); print(x - 1.0); 0 }", "in_float.ray");
    compare_source("fn main() -> int { print(true && false); print(true || false); print(!false); 0 }", "in_logic.ray");
    compare_source("fn main() -> int { print(1 < 2); print(2 <= 2); print(3 > 4); print('a' < 'b'); 0 }", "in_cmp.ray");
    compare_source("fn main() -> int { print(\"foo\" + \"bar\"); 0 }", "in_concat.ray");
    compare_source("fn main() -> int { if (1 == 1) { print(\"si\"); } else { print(\"no\"); } 0 }", "in_if.ray");
}

#[test]
fn variables_y_mutation() {
    compare_source(
...
#[test]
fn calls_y_recursion() {
    compare_source("fn double(x: int) -> int { x * 2 } fn main() -> int { print(double(21)); 0 }", "in_call.ray");
    compare_source(
...
fn ufcs_snippets() {
    ...
    compare_source(
        "fn double(x: int) -> int { x * 2 } fn head<T>(xs: [T]) -> T { xs[0] } fn main() -> int { let xs = [10, 20, 30]; print(xs.len()); print(xs.head()); print((5).double().double()); 0 }",
        "in_ufcs.ray",
    );
    compare_source(
...
fn traits_y_bounds_snippets() {
    ...
    compare_source(
        "trait S { fn name(self) -> string; fn greet(self) -> string { self.name() } } struct A { } impl S for A { fn name(self) -> string { \"a\" } } struct B { } impl S for B { fn name(self) -> string { \"b\" } fn greet(self) -> string { \"hello\" } } fn main() -> int { print(A { }.greet()); print(B { }.greet()); 0 }",
        "in_default.ray",
    );
}
...
fn dyn_y_derive_snippets() {
    ...
    compare_source(
        "trait An { fn voice(self) -> string; fn di(self) -> int { print(self.voice()); 1 } } struct Pe { } impl An for Pe { fn voice(self) -> string { \"guau\" } } struct Ga { } impl An for Ga { fn voice(self) -> string { \"miau\" } } fn coro(xs: [dyn An]) -> int { var n = 0; var i = 0; while (i < xs.len()) { n = n + xs[i].di(); i = i + 1; } n } fn main() -> int { let xs: [dyn An] = [Pe { }, Ga { }]; coro(xs) }",
        "in_dyn.ray",
    );
...
fn map_filter_fold_snippets() {
    ...
    compare_source(
        "fn double(x: int) -> int { x * 2 } fn par(x: int) -> bool { x % 2 == 0 } fn sum(a: int, b: int) -> int { a + b } fn main() -> int { let xs = [1, 2, 3, 4]; print(xs.map(double).filter(par).fold(0, sum)); 0 }",
        "in_mff_ufcs.ray",
    );
    // Pipeline + closures anónimos en línea.
    compare_source(
        "fn sum(a: int, b: int) -> int { a + b } fn main() -> int { let xs = [1, 2, 3, 4, 5]; print(xs |> map(fn(x: int) -> int { x * x }) |> filter(fn(x: int) -> bool { x > 5 }) |> fold(0, sum)); 0 }",
        "in_mff_pipe.ray",
    );
...
fn map_builtins() {
    ...
    compare_source(
        "fn main() -> int { \
            ...
            m.insert(\"one\", 1); m.insert(\"dos\", 2); m.insert(\"tres\", 3); m.insert(\"dos\", 22); \
            print(m.len()); print(m.contains_key(\"one\")); print(m.contains_key(\"x\")); \
...
fn assert_y_sort() {
    ...
    compare_source(
        "struct P { v: int } \
         impl Ord for P { fn less(self, other: P) -> bool { self.v < other.v } } \
    ...
    compare_file("examples/data/mapa.ray");
}
...
fn io_de_files() {
    ...
    compare_source(
        "fn main() -> int { \
            let w = __write_file(\"/tmp/raylang_d_oracle.txt\", \"abc\\ndef\"); print(w[0]) \
            let r = __read_file(\"/tmp/raylang_d_oracle.txt\"); if (r[0] == \"ok\") { print(r[1]) } else { print(\"ERR\") } \
            let r2 = __read_file(\"/tmp/raylang_d_no_existe_zzz.txt\"); if (r2[0] == \"ok\") { print(r2[1]) } else { print(\"ERR\") } \
            print(__exists(\"/tmp/raylang_d_oracle.txt\")); print(__exists(\"/tmp/raylang_d_no_existe_zzz.txt\")); \
            0 \
        }",
        "in_io.ray",
    );
...
fn compare_modules(dir: &str, files: &[(&str, &str)], entry: &str) {
    let mut base = std::env::temp_dir();
    base.push(dir);
    std::fs::create_dir_all(&base).expect("crea el directory temporal");
    for (name, src) in files {
        let mut path = base.clone();
        path.push(name);
        let mut f = std::fs::File::create(&path).expect("crea el módulo temporal");
        f.write_all(src.as_bytes()).expect("escribe el módulo temporal");
    }
    let mut ep = base.clone();
    ep.push(entry);
    let abs = ep.to_str().expect("path utf-8");
    let (so_r, code_r) = run_rust(abs);
    let (so_s, code_s) = run_selfhost(abs);
    assert_eq!(so_s, so_r, "stdout difiere en módulos {dir}/{entry}");
    assert_eq!(code_s, code_r, "código de output difiere en módulos {dir}/{entry}");
}

#[test]
fn modules_functions() {
    // M14.7a: cruce de FUNCIONES entre módulos vía `from M import …` (sin tipos todavía).
    compare_modules(
        "rl_m7a_basico",
        &[
            ("helper.ray", "pub fn double(x: int) -> int { x * 2 }\npub fn square(x: int) -> int { x * x }\n"),
            ("main.ray", "from helper import double, square;\nfn main() -> int { print(double(21)); print(square(5)); 0 }\n"),
        ],
        "main.ray",
    );
    ...
    compare_modules(
        ...
        &[
            ...
            ("b.ray", "from c import triple;\npub fn applies(x: int) -> int { triple(x) + 1 }\n"),
            ("main.ray", "from b import applies;\nfn main() -> int { let xs = [1, 2, 3]; print(xs.map(applies)); 0 }\n"),
        ],
        "main.ray",
    );
}
...
fn modules_types() {
    ...
    compare_modules(
        "rl_m7b_traits",
        &[
            ("shapes.ray", "pub trait Medible { fn measure(self) -> int; }\n\
                pub struct Caja<T> { valor: T, etiqueta: int }\n\
                pub struct Cuadrado { lado: int }\n\
                impl Medible for Cuadrado { fn measure(self) -> int { self.lado * self.lado } }\n\
                pub fn envolver(v: int) -> Caja<int> { Caja { valor: v, etiqueta: 0 } }\n"),
            ("main.ray", "from shapes import Medible, Caja, Cuadrado, envolver;\n\
                fn describir(m: dyn Medible) -> int { m.measure() }\n\
                fn main() -> int { let c = Cuadrado { lado: 5 }; print(c.measure()); print(describir(c)); let b: Caja<int> = envolver(42); print(b.valor); 0 }\n"),
        ],
        "main.ray",
    );
