...
//! un módulo raylang (`pub fn render(…) -> string` (M103: nombre fijo, el módulo namespacea)) que se importa como cualquier módulo.
...
fn main() -> int {
    print(list.render("Informe & data", false, ["a<b", "c"], 2));
    0
}
"#;
...
fn generates_and_renders_on_both_engines() {
    ...
    assert!(generated.contains("pub fn render(titulo: string, admin: bool, filas: [string], total: int) -> string"), "{generated}");
...
fn include_and_import_compose_views_and_layout() {
    ...
    // El include por RUTA: sin `{% import %}` manual ni conocer la función generada.
    std::fs::write(app.join("vistas/pagina.ray.html"),
        "{% params names: [string] %}<ul>{% for n in names %}{% include vistas/tarjeta(n) %}{% endfor %}</ul>\n").unwrap();
    std::fs::write(app.join("main.ray"),
        "import vistas/pagina;\nimport vistas/layout;\n\nfn main() -> int {\n    print(layout.render(\"Equipo\", pagina.render([\"Ada\", \"Lin<us\"])));\n    0\n}\n").unwrap();
...
fn extends_block_y_let_end_to_end() {
    ...
    std::fs::write(app.join("main.ray"),
        "import vistas/pagina;\n\nfn main() -> int {\n    print(pagina.render(\"Lista\", [1, 2, 3]));\n    0\n}\n").unwrap();
